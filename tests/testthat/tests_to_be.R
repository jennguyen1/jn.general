...
cleanup <- function(d){
  if(is.data.frame(d)){
    out <- tibble::as.tibble(d)
  } else{
    out <- lapply(d, tibble::as.tibble)
    names(out) <- names(d)
  }
  out
}
...
test_that("to_be splits data frames", {
  ...
  slice_evens <- dplyr::slice(d, evens) %>% cleanup()
  slice_odds <- dplyr::slice(d, odds) %>% cleanup()
  to_be_slice <- to_be(d, dplyr::slice, evens) %>% cleanup()
  expect_equal(to_be_slice$to_be, slice_evens)
  expect_equal(to_be_slice$not_to_be, slice_odds)
  
  eq <- subset(d, x == y) %>% cleanup()
  neq <- subset(d, x != y) %>% cleanup()
  to_be_subset <- to_be(d, subset, x == y) %>% cleanup()
  expect_equal(to_be_subset$to_be, eq)
  expect_equal(to_be_subset$not_to_be, neq)
  
  cols <- c("x", "y")
  xy <- dplyr::select(d, dplyr::one_of(cols)) %>% cleanup()
  z <- dplyr::select(d, -dplyr::one_of(cols)) %>% cleanup()
  to_be_select <- to_be(d, dplyr::select, dplyr::one_of(cols)) %>% cleanup()
  expect_equal(to_be_select$to_be, xy)
  expect_equal(to_be_select$not_to_be, z)
  
  n <- 5
  hd <- head(d, n) %>% cleanup()
  tl <- tail(d, nrow(d) - n) %>% cleanup()
  to_be_head <- to_be(d, head, n) %>% cleanup()
  expect_equal(to_be_head$to_be, hd)
  expect_equal(to_be_head$not_to_be, tl)
  
  dups <- view_duplicated(d, x, y) %>% cleanup()
  no_dups <- remove_duplicated(d, x, y, opt_delete = "all") %>% cleanup()
  to_be_dups <- to_be(d, view_duplicated, x, y) %>% cleanup()
  expect_equal(to_be_dups$to_be, dups)
  expect_equal(to_be_dups$not_to_be, no_dups)
  
  n <- 3
  set.seed(1)
  random <- dplyr::sample_n(d, n) %>% cleanup()
  not_in_random <- subset(d, !(id %in% random$id)) %>% cleanup()
  set.seed(1)
  to_be_random <- to_be(d, dplyr::sample_n, n) %>% cleanup()
  expect_equal(to_be_random$to_be, random)
  expect_equal(to_be_random$not_to_be, not_in_random)
})

test_that("to_be empty data frames", {
  to_be_empty_rows <- to_be(d, subset, x != 100) %>% cleanup()
  expect_equal(to_be_empty_rows$to_be, d)
  expect_equal(to_be_empty_rows$not_to_be, empty_df)
  
  empty_cols <- dplyr::select(d, dplyr::one_of("a")) %>% cleanup()
  to_be_empty_cols <- to_be(d, dplyr::select, dplyr::one_of("a")) %>% cleanup()
  expect_equal(to_be_empty_cols$to_be, empty_cols)
  expect_equal(to_be_empty_cols$not_to_be, d)
  
  to_be_empty <- to_be(empty_df, subset, x == 1) %>% cleanup()
