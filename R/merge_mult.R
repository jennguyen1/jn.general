...
merge_mult <- function(..., f, by, suffixes = letters){
  l <- list(...)
  if( !is.data.frame(l[[1]]) ) l <- l[[1]] # TODO did they apply a second list?

  ## checks
  assertthat::assert_that(all(unlist(lapply(l, is.data.frame))), msg = "Inputs should be data frames or a list of data frames")
